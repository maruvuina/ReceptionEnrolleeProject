...
function getDepartmentValuesBySpeciality(index){
    var sSpecialityValues;
    if (userLang === "ru_RU") {
        sSpecialityValues = specialityValuesRussian[index];
    } else if (userLang === "en_US") {
        sSpecialityValues = specialityValuesEnglish[index];
    } else if (userLang === "be_BY") {
        sSpecialityValues = specialityValuesBelarussian[index];
    } else {
        sSpecialityValues = specialityValuesEnglish[index];
    }
    return sSpecialityValues.split(","); // Преобразуем строку в массив специальностей
}

// Главная ф-ция, выводящая динамически список специальностей
function changeDepartmentValues(index) {
    ...
    oSpecialityList.length = 0; // удаляем все элементы из списка специальностей
...
// Инициализируем изменение элементов в списке специальностей,
// в зависимости от текущего факультета
